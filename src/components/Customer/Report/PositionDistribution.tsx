  ...
  return (
    ...
    >
      ...
      <div
        ...
        role="img"
        aria-label={`การกระจาย: ${BUCKETS.map(
          (b) => `${b.label} ${dist[b.key]}`,
        ).join(", ")}`}
      >
        {BUCKETS.map((bucket) => {
      ...
      <ul className="flex flex-wrap gap-x-4 gap-y-1.5">
        {BUCKETS.map((bucket) => {
          ...
          return (
            ...
            >
              <span
                className={cn("size-2.5 rounded-sm", bucket.className)}
                aria-hidden="true"
              />
