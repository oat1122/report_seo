...
const chartConfig = buildChartConfig([
  { key: "rate", label: "Top 10 %", color: "var(--info)" },
]);

export const KdSuccessRateBar = ({
  keywords,
  topN = 10,
}: KdSuccessRateBarProps) => {
  const rows = useMemo(() => {
    ...
    <Card className="h-full">
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          % keyword ที่ติด Top {topN} แยกตามความยาก
        </p>
      </CardHeader>
      <CardContent>
        {!hasAny ? (
