...
const phaseClassMap: Record<
  DomainPhase,
  { dot: string; badge: string; progress: string }
> = {
  establishing: {
    dot: "bg-info",
  ...
  if (!metrics) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">ไม่มีข้อมูลโดเมน</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      ...
      <CardContent>
        <div className="mb-3 flex items-baseline justify-between gap-3">
          <span className="text-3xl font-extrabold tabular-nums md:text-4xl">
            {ageStr}
          </span>
