...
export const BracketTransitionsSankey = () => {
  ...
  const layout = useMemo(() => {
    ...
    const gapTotal = (side: SankeyNode[]) =>
      side.length > 1 ? NODE_GAP * (side.length - 1) : 0;
  ...
  const linkPaths = useMemo(() => {
    ...
    const pxPerCount =
      Math.min(
        layout.from.reduce((s, n) => s + n.h, 0) /
          Math.max(layout.from.reduce((s, n) => s + n.total, 0), 1),
        layout.to.reduce((s, n) => s + n.h, 0) /
          Math.max(layout.to.reduce((s, n) => s + n.total, 0), 1),
    ...
    return sorted.map((link, idx) => {
      ...
      const improved =
        order.indexOf(link.toBracket) < order.indexOf(link.fromBracket);
      const degraded =
        order.indexOf(link.toBracket) > order.indexOf(link.fromBracket);
              ...
              {linkPaths.map(
                (lp) =>
                  lp && (
                    ...
                    >
                      <title>
                        {lp.link.count} keywords:{" "}
                        {bracketLabel(lp.link.fromBracket)} →{" "}
