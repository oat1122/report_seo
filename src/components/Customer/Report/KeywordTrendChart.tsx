...
export const KeywordTrendChart: React.FC<KeywordTrendChartProps> = ({
    ...
    selectedKeywords.forEach((keyword) => {
      ...
      if (current) {
        ...
        const dateExists = records.some(
          (r) => r.date.toISOString().split("T")[0] === dayKey,
        );
        if (!dateExists) {
          records.push({
            date: latestDate,
  ...
  const wideRows = useMemo<WideRow[]>(() => {
    ...
    selectedKeywords.forEach((keyword) => {
      ...
      recs.forEach((r) => {
        ...
        row[`pos_${keyword}`] =
          r.position == null ? null : clampPosition(r.position);
  ...
  const donutConfig = useMemo(
    () =>
      buildChartConfig(
        donutData.map((d) => ({ key: d.label, label: d.label, color: d.color })),
      ),
    [donutData],
  );
  ...
  const isSinglePoint = useMemo(() => {
    ...
    const maxLen = Math.max(
      0,
      ...Array.from(recordsByKeyword.values()).map((r) => r.length),
    );
  ...
  const positionTooltipFormatter = (
    ...
    const formatted =
      typeof display === "number" && display > 0 ? `#${display}` : "-";
    return [formatted, k];
  };
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setFocusedKeyword(isFocused ? null : keyword)
                }
          ...
          <div className="flex flex-col gap-4 md:col-span-2">
              ...
              {hasPositionData ? (
                ...
                >
                  ...
                  >
                    ...
                    {selectedKeywords.map((k) => {
                      const baseColor =
                        keywordColorMap.get(k) || CHART_COLORS.primary;
                      const isDim =
                        focusedKeyword !== null && focusedKeyword !== k;
                      return (
                        <Line
                          key={k}
                          type="monotone"
                ...
                <ChartEmptyState
                  message="ยังไม่มีข้อมูล Position"
                  height="220px"
                />
              )}
            </div>
            ...
            <div className="rounded-xl border border-border bg-background p-3">
              ...
              {hasTrafficData ? (
                ...
                <ChartEmptyState
                  message="ยังไม่มีข้อมูล Traffic"
                  height="220px"
                />
              )}
            </div>
            ...
            >
              <PieChart>
                ...
                <Pie
                  ...
                  onClick={(d) => {
                    const label =
                      (d as { label?: string; payload?: { label?: string } })
                        .label ??
                ...
                >
                  {donutData.map((d) => (
                    <Cell
                      ...
                      fill={d.color}
                      opacity={
                        focusedKeyword && focusedKeyword !== d.label ? 0.35 : 1
                      }
                ...
                return (
                  ...
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setFocusedKeyword(isFocused ? null : item.label)
                      }
