...
export const useAddKeyword = () => {
  ...
  return useMutation<KeywordReport, Error, { customerId: string; keyword: KeywordFormData }>({
...
export const useDeleteKeyword = () => {
  const queryClient = useQueryClient()
  return useMutation<void, Error, { customerId: string; keywordId: string }>({
...
export const useToggleKeywordHistoryVisibility = () => {
  ...
  >({
    ...
    onMutate: async (variables) => {
      ...
      const previousCombined = queryClient.getQueryData<CombinedHistoryData>(queryKey)
    ...
    onError: (_err, variables, context) => {
      if (context?.previousCombined) {
        queryClient.setQueryData(['history', variables.customerId], context.previousCombined)
      }
    },
