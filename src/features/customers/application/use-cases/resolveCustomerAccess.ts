...
export function resolveCustomerAccessUseCase(
  ...
  return async (query: CustomerAccessQuery): Promise<CustomerAccessContext> => {
...
async function locate(customers: CustomerRepository, query: CustomerAccessQuery) {
  ...
  if ('byKeywordId' in query) return customers.findByKeywordId(query.byKeywordId)
  if ('byRecommendId' in query) return customers.findByRecommendId(query.byRecommendId)
