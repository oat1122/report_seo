...
export const useListPaymentPlans = (customerId: string, status?: string) =>
...
export const useCreatePaymentPlan = () => {
  ...
  return useMutation<PaymentPlan, Error, { customerId: string; plan: CreatePaymentPlanInput }>({
...
export const useCancelPaymentPlan = () => {
  const queryClient = useQueryClient()
  return useMutation<PaymentPlan, Error, { customerId: string; planId: string }>({
