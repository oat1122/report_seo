...
import { MAX_AI_OVERVIEW_IMAGES, type AiOverviewCreateInput } from '../../schemas'
import type { AiOverviewRepository } from '../ports/AiOverviewRepository'
import type { ImageStorage } from '../ports/ImageStorage'

export function createAiOverviewUseCase(repo: AiOverviewRepository, storage: ImageStorage) {
  return async (
    ...
    if (files.length > MAX_AI_OVERVIEW_IMAGES) {
      throw new BadRequestError(`อัปโหลดรูปภาพได้สูงสุด ${MAX_AI_OVERVIEW_IMAGES} รูป`)
    }
