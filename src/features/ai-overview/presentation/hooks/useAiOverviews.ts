...
export const useAddAiOverview = () => {
  ...
  return useMutation<AiOverview, Error, { customerId: string; formData: FormData }>({
...
export const useUpdateAiOverview = () => {
  const queryClient = useQueryClient()
  return useMutation<AiOverview, Error, { customerId: string; id: string; formData: FormData }>({
