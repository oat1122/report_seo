...
const UPLOAD_CATEGORY = 'next-steps' as const
...
export class LocalNextStepImageStorage implements ImageStorage {
  async validateAndWrite(files: File[]): Promise<SavedImage[]> {
    ...
    try {
      ...
      // write ล้มกลางทาง — ลบรูปที่เพิ่งเขียนไปแล้วกัน orphan file
