...
import { useGenerateDocument, useGenerateAllForCycle } from '../../hooks/useDocuments'
...
export function DocumentGeneratorPanel({ customerId }: Props) {
  ...
  const itemsTotal = useMemo(
    () =>
      template
        ? template.items.reduce((sum, i) => sum + Number(i.quantity) * Number(i.unitPrice), 0)
        : 0,
    [template],
  )
  ...
  const amountMismatch =
    hasCycle && selectedCycle && template && Math.abs(itemsTotal - selectedCycle.amount) > 0.01
  ...
  const isPending = generateMutation.isPending || generateAllMutation.isPending

  return (
    <Card>
      <CardHeader>
        <CardTitle>สร้างเอกสาร</CardTitle>
        <CardDescription>เลือกรอบจ่ายเงิน (ถ้ามี) แล้วสร้าง PDF</CardDescription>
      </CardHeader>
      <CardContent>
        <FieldGroup>
          {cycles.length > 0 && (
          ...
          {hasCycle && !templateId && (
            <div className="border-destructive/50 bg-destructive/5 flex items-start gap-2 rounded-md border p-3 text-sm">
              <AlertTriangle className="text-destructive mt-0.5 size-4 shrink-0" />
              <p className="text-destructive">แผนชำระนี้ยังไม่ได้เลือก template เอกสาร</p>
            </div>
          )}
          ...
          {amountMismatch && (
            <div className="border-destructive/50 bg-destructive/5 flex items-start gap-2 rounded-md border p-3 text-sm">
              <AlertTriangle className="text-destructive mt-0.5 size-4 shrink-0" />
              <div>
                <p className="text-destructive font-medium">ยอดรวมไม่ตรงกับรอบจ่ายเงิน</p>
                <p className="text-muted-foreground">
                  ยอดรายการ:{' '}
                  {itemsTotal.toLocaleString('th-TH', {
                    minimumFractionDigits: 2,
