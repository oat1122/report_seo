      ...
      <CardContent className="flex flex-col gap-5 pt-6">
        ...
        {isLoading ? (
          ...
          <div className="border-border flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed p-12 text-center">
            ...
            <p className="text-muted-foreground text-sm">ยังไม่มี template</p>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {templates.map((t) => (
...
function TemplateCard({ tpl, href, onDelete }: TemplateCardProps) {
  ...
  return (
    <Card
      className={`group hover:ring-foreground/20 relative transition-all hover:shadow-md ${
        tpl.isActive ? '' : 'opacity-60'
      }`}
    >
      <CardHeader className="flex flex-row items-start justify-between gap-3 pb-0">
        <Link href={href} className="flex min-w-0 flex-1 items-center gap-3">
          <span className="bg-secondary/15 text-foreground ring-secondary/30 group-hover:bg-secondary/30 flex size-11 shrink-0 items-center justify-center rounded-lg ring-1 transition-colors">
            <FileStack className="size-5" />
          </span>
          <div className="flex min-w-0 flex-col gap-0.5">
            <span className="line-clamp-1 text-base leading-snug font-semibold">{tpl.name}</span>
            <span className="text-muted-foreground text-xs">{tpl.durationMonths} เดือน</span>
          </div>
        </Link>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              variant="ghost"
              aria-label="เมนู"
              className="-mt-1.5 -mr-1.5 opacity-60 transition-opacity group-hover:opacity-100"
            >
        ...
        <Link href={href} className="block">
          <p
            className={`line-clamp-2 min-h-[2.5rem] text-sm leading-relaxed ${
              hasDescription ? 'text-foreground/75' : 'text-muted-foreground/60 italic'
            }`}
          >
      ...
      <CardFooter className="bg-muted/30 flex items-center justify-between gap-2 py-2.5">
        ...
        <span className="text-muted-foreground text-[11px]">อัปเดต {updatedLabel}</span>
      </CardFooter>
    </Card>
  )
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!name.trim() || createMut.isPending}>
            สร้าง
          </Button>
        </DialogFooter>
      </DialogContent>
