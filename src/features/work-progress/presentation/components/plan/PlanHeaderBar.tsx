  ...
  return (
    <div className="border-border bg-card flex flex-col gap-3 rounded-lg border p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
              <Badge variant="secondary">{PERIOD_LABEL[data.periodType] ?? data.periodType}</Badge>
              {data.year && <Badge variant="outline">{data.year}</Badge>}
              {data.packageName && <Badge variant="outline">{data.packageName}</Badge>}
              {data.isArchived && (
      ...
      <div className="flex items-center gap-3">
        <span className="text-muted-foreground text-xs">ความคืบหน้ารวม</span>
        <Progress value={overall} className="flex-1" />
        <span className="min-w-12 text-right text-sm font-medium tabular-nums">{overall}%</span>
      </div>
