...
export function PlanList({ userId, basePath, readOnly }: PlanListProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!hasPlans ? (
        <EmptyPlansState onCreate={() => setCreateOpen(true)} readOnly={readOnly} />
      ) : (
      ...
      {!readOnly && (
        <>
          <CreatePlanDialog userId={userId} open={createOpen} onOpenChange={setCreateOpen} />
...
function PlanCard({ plan, href, onEdit, onArchiveToggle, onDelete, readOnly }: PlanCardProps) {
  return (
    <Card
      className={
        plan.isArchived
    ...
    >
      <CardHeader className="flex flex-row items-start justify-between gap-2 pb-2">
        ...
        >
          <ClipboardList className="text-muted-foreground mt-0.5 size-5" />
          <div className="flex flex-col">
            <span className="line-clamp-2 font-semibold">{plan.title}</span>
            {plan.packageName && (
              <span className="text-muted-foreground text-xs">{plan.packageName}</span>
            )}
          </div>
        </Link>
        {!readOnly && (
      ...
      <CardContent>
        <Link href={href} className="block">
          ...
          {plan.note && (
            <p className="text-muted-foreground mt-3 line-clamp-2 text-xs">{plan.note}</p>
          )}
        </Link>
      </CardContent>
    </Card>
