...
export const useAddItem = () => {
  ...
  return useMutation<WorkProgressItemWithMarks, Error, PlanCtx & { body: AddItemInput }>({
...
export const useDeleteItem = () => {
  ...
  return useMutation<void, Error, PlanCtx & { itemId: string }>({
    mutationFn: async ({ userId, planId, itemId }) => {
      await axios.delete(`/customers/${userId}/work-progress/${planId}/items/${itemId}`)
    },
    onSuccess: (_d, vars) => invalidate(vars),
  })
}
...
export const useReorderItems = () => {
  ...
  return useMutation<void, Error, PlanCtx & { body: ReorderItemsInput }>({
    mutationFn: async ({ userId, planId, body }) => {
      await axios.post(`/customers/${userId}/work-progress/${planId}/items/reorder`, body)
    },
    onSuccess: (_d, vars) => invalidate(vars),
  })
}
...
export const useAssignItem = () => {
  const invalidate = useInvalidatePlan()
  return useMutation<unknown, Error, PlanCtx & { itemId: string; body: AssignItemInput }>({
    mutationFn: async ({ userId, planId, itemId, body }) => {
...
export const useBulkUpdateItemStatus = () => {
  const invalidate = useInvalidatePlan()
  return useMutation<{ count: number }, Error, PlanCtx & { body: BulkUpdateItemStatusInput }>({
    mutationFn: async ({ userId, planId, body }) => {
...
export const useBulkDeleteItems = () => {
  const invalidate = useInvalidatePlan()
  return useMutation<{ count: number }, Error, PlanCtx & { body: BulkDeleteItemsInput }>({
    mutationFn: async ({ userId, planId, body }) => {
      const { data } = await axios.post<ApiData<{ count: number }>>(
...
export const useBulkSetPeriodAcrossItems = () => {
  const invalidate = useInvalidatePlan()
  return useMutation<{ count: number }, Error, PlanCtx & { body: BulkSetPeriodAcrossItemsInput }>({
    mutationFn: async ({ userId, planId, body }) => {
      const { data } = await axios.post<ApiData<{ count: number }>>(
