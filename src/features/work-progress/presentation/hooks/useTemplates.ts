...
const detailKey = (id: string) =>
  ["workProgress", "template", id] as const;
...
export const useUpdateTemplate = () => {
  ...
  return useMutation<
    WorkProgressTemplate,
    Error,
    { id: string; body: UpdateTemplateInput }
  >({
...
export const useAddTemplateItem = () => {
  ...
  return useMutation<
    unknown,
    Error,
    { templateId: string; body: AddTemplateItemInput }
  >({
    mutationFn: async ({ templateId, body }) => {
      const { data } = await axios.post(
        `/work-progress/templates/${templateId}/items`,
        body,
      );
      return data;
    },
...
export const useReorderTemplateItems = () => {
  ...
  return useMutation<
    unknown,
    Error,
    { templateId: string; body: ReorderTemplateItemsInput }
  >({
    mutationFn: async ({ templateId, body }) => {
      const { data } = await axios.post(
