...
function applyMarkPatch(
  ...
  return {
    ...
    items: prev.items.map((item) => {
      ...
      const existingIdx = item.periodMarks.findIndex(
        (m) => m.periodId === body.periodId,
      );
  ...
  return {
    ...
    items: prev.items.map((item) =>
      item.id === ctx.itemId
        ? {
            ...item,
            periodMarks: item.periodMarks.filter(
              (m) => m.periodId !== periodId,
            ),
          }
        : item,
    ),
...
export const useSetPeriodMark = () => {
  ...
  >({
    ...
    onMutate: async (vars) => {
      ...
      qc.setQueryData<WorkProgressPlanDetail>(
        key as unknown as string[],
        (old) => applyMarkPatch(old, vars, vars.body, vars.markType),
      );
      return { key, prev };
    },
    onError: (_e, _v, ctx) => {
...
export const useClearPeriodMark = () => {
  ...
  >({
    ...
    onMutate: async (vars) => {
      const key = planDetailKey(vars.userId, vars.planId);
      await qc.cancelQueries({ queryKey: key as unknown as string[] });
      const prev = qc.getQueryData<WorkProgressPlanDetail>(
        key as unknown as string[],
      );
      qc.setQueryData<WorkProgressPlanDetail>(
        key as unknown as string[],
        (old) => applyClearPatch(old, vars, vars.periodId),
      );
      return { key, prev };
    },
    onError: (_e, _v, ctx) => {
