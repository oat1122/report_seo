...
function useInvalidate(kind: MasterKindCode) {
  ...
  return () => qc.invalidateQueries({ queryKey: [...kindQueryKey[kind]] as string[] })
}
...
export const useUpdateCategory = () => {
  const invalidate = useInvalidate('category')
  return useMutation<WorkProgressCategory, Error, { id: string; body: UpdateCategoryInput }>({
...
export const useUpdateStatus = () => {
  const invalidate = useInvalidate('status')
  return useMutation<WorkProgressStatus, Error, { id: string; body: UpdateStatusInput }>({
    mutationFn: async ({ id, body }) => {
...
export const useUpdateMarkType = () => {
  const invalidate = useInvalidate('markType')
  return useMutation<WorkProgressMarkType, Error, { id: string; body: UpdateMarkTypeInput }>({
    mutationFn: async ({ id, body }) => {
