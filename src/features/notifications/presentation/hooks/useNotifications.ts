...
export function useNotifications() {
  return useInfiniteQuery<ApiPaginated<Notification>>({
    ...
    queryFn: async ({ pageParam }) => {
      const { data } = await axios.get<ApiPaginated<Notification>>(
        "/notifications",
...
export function useUnreadCount() {
  return useQuery<number>({
    ...
    queryFn: async () => {
      const { data } = await axios.get<ApiData<{ count: number }>>(
...
export function useNotificationPreferences() {
  return useQuery<PreferenceWithLabel[]>({
    ...
    queryFn: async () => {
      const { data } = await axios.get<ApiData<PreferenceWithLabel[]>>(
