...
const trendStyle: Record<
  Trend,
  { bar: string; text: string; Icon: typeof TrendingUp }
> = {
  up: {
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between">
        ...
        {trend === "new" ? (
          <Badge className="gap-1 bg-info/10 text-xs font-semibold text-info">
      ...
      <div className="relative h-2 overflow-visible rounded-full bg-border">
        <div
        ...
        {isOverflow && (
          <>
            <div className="absolute inset-y-0 left-0 w-full rounded-full bg-gradient-to-r from-warning via-warning/70 to-warning opacity-70" />
            <div className="absolute -top-0.5 -right-1 size-3 animate-pulse rounded-full bg-warning shadow-[0_0_8px_color-mix(in_srgb,var(--warning)_60%,transparent)]" />
          </>
        )}
      </div>

      {isOverflow && (
        <p className="mt-1 text-right text-[0.65rem] font-bold text-warning">
