...
const computeAvgPosition = (
  keywords: Array<{ position: number | null }>,
): number | null => {
  const ranked = keywords.filter(
    (k): k is { position: number } =>
      k.position !== null && k.position > 0,
  );
...
export const computeKpiSnapshots = (
  ...
  const prevTotal = getValueAtOrBefore(
    metricsHistory,
    daysAgo,
    (r) => r.organicKeywords,
  );
  const totalSparkline = buildSparkline(
    metricsHistory,
    (r) => r.organicKeywords,
  );
  ...
  const prevAvg = computeAvgPosition(
    Array.from(historicalByKeyword.values()),
  );
...
export const computeRoiHeadline = (
  ...
  const currentTraffic =
    metricsHistory.length > 0 ? metricsHistory[0].organicTraffic : null;
  const previousTraffic = getValueAtOrBefore(
    metricsHistory,
    period,
    (r) => r.organicTraffic,
  );
  ...
  const { gainers, losers } = computeTopMovers(
    keywordHistory,
    currentKeywords,
    period,
    1000,
  ...
  const hasData =
    trafficPctChange !== null || gainers.length > 0 || losers.length > 0;
...
export const computeCoverageStats = (
  ...
  const lastUpdated =
    metricsHistory.length > 0
      ? new Date(metricsHistory[0].dateRecorded)
      : null;
  return {
...
export const computeAuthorityRadar = (
  ...
  const trustFromSpam = (spam: number) =>
    Math.max(0, Math.min(100, 100 - spam * 10));
  ...
  return [
    ...
    {
      axis: "Backlinks",
      current: normalizeToScore100(
        current.backlinks,
        BACKLINKS_FULL_SCALE,
        "log10",
      ),
      previous:
        prevBacklinks !== null
    ...
    },
    {
      ...
      previous:
        prevTraffic !== null
          ? normalizeToScore100(prevTraffic, TRAFFIC_FULL_SCALE, "log10")
          : null,
 ...
 */
export const computeDomainPhase = (
  years: number,
  months: number,
): DomainPhaseInfo => {
...
export const computeBacklinkRatio = (
  backlinks: number,
  refDomains: number,
): number | null => {
...
export const computeKdSuccessRate = (
  ...
  return levels.map((level) => {
    const inLevel = keywords.filter(
      (k) => String(k.kd).toUpperCase() === level,
    );
...
export const computeAiOverviewWeeklyCounts = (
  ...
  const earliestWeekStart = new Date(
    latestWeekStart.getTime() - (weeks - 1) * WEEK_MS,
  );
...
export const computeSparklineTopN = (
  ...
  return sorted.map((kw) => {
    ...
    const deltaPct =
      prevTraffic > 0 ? ((currentTraffic - prevTraffic) / prevTraffic) * 100 : null;
...
export const computeBracketTransitions = (
  ...
  for (const b of order) {
    const fromTotal = links
      .filter((l) => l.fromBracket === b)
